...
begin
  { True = Gerar a prod list, False = não gerar a prod list }
  with Prod_List do
  begin
    Close;
    ParamByName('cod_empresa').Value := vgCod_Empresa;
...
{ Importação dos Itens da Production List }
...
begin
  ...
  begin
    ...
    { Comparar Confirmação da Planilha com Confirmação do Pedido }
    ...
    { indicar que houve alteração cadastral para o produto - sem preenchimento colocado pela importação }
    { -4142 é o valor que o Excel coloca na célula para indicar "sem preenchimento" GOIABA }
    ...
    { Zerar sequencia do item quando não existir mais na fatura }
    ...
    { BD - Número da Sequencia de Origem }
    try
    ...
    { Verificar Peso Unitário do Produto embalagem esp/padrão }
    if (Mesclar = 'N') then
    begin
    ...
    begin
      ...
      { Saldo Pendente = 0 ou item não encontrado }
      if (QtdSaldo = 0) then
      begin
        Erro := True;
        Erros := SetBit(Erros, 0);
      ...
      { Marcar item da Fatura para exclusão }
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              Post;
              { Romaneando menor que o Saldo, irá abrir um novo item }
              if (qtRomanear < QtdSaldo) then
              begin
    ...
    { Saldos dos itens da planilha, Fatura não Existe ainda }
    ...
    end; { Fatura não existe }

    { Preço da planilha <> do Pedido }
    ...
    if (Mesclar = 'N') then // peso únitário de cada item da embalagem especial
    ...
    { CALCULAR CAMPOS PARA COMPARAÇÕES }
...
begin
  ...
  Erro := False; { indicar ERRO na importação da Planilha }

  { Abrir o Excel de Production List  }
  FileName := GetDirProdList;

  { obter Número da Prod List do Cliente/Fornecedor }
  ...
  { Limpar Prod_list_Itens - excluir a importação anterior  }
  ...
  { Recalcular Caixas Standard para produtos com alteração cadastral }
  ...
  { Atualizar a planilha somente quando NÃO ocorrer ERROS }
  if Not(Erro) then
  begin
    ...
    { Refaturar uma Fatura da Prod List não embarcada }
  ...
  { Hablitar/Desabilitar o botão de Faturar }
  actFaturar.Enabled := Not(Erro);

  { Mostrar mensagem sobre situação após a importação }
  if Erro then Application.MessageBox('Importação efetuada com erros', 'Erro', MB_OK+MB_ICONERROR)
          else Application.MessageBox('Importação executada com sucesso e sem erros', 'Informação', MB_OK+MB_ICONINFORMATION);
...
begin
  ...
  begin
    Erros := 0;
    flagAlteraCadastro := False; // indicar quando é necessário atualizar Pedidos/Faturas/ProdList
    ...
    begin
      ...
      { ignorar produto não encontrado }
      ...
      { Quando Emb. Normal ou Emb. Especial não mesclada }
      if ((Erros = 0) and (Mesclar = 'N')) then // atualizar quando não houver erros na importação
      begin
        ...
        if (Not(EmbEsp) and (sMD5 <> sMD5Excel)) then // Somente embalagem padrão: Embalagens: Volume e Peso Bruto da Caixa Master
        begin
          ExcelWorksheet.Range['D'+ inttostr(linha),'D'+inttostr(linha)].Interior.ColorIndex :=  xlNone;
...
begin
  ...
  begin
    ...
    begin
      ...
      { Obter Número do Pedido no Sistema }
      with dmCOM.GetPedido(Pedido, 'P') do
      begin
        NumPedido := Numero;
      end;

      if (NumPedido = 0) then Continue; { ignorar pedido não encontrado }
...
begin
  ...
  begin
    Application.MessageBox('Mudanças em Pedidos ou Faturas alteraram a Production List.' + #13 + 'Por favor gere as planilhas de Production List novamente.', 'Informação', MB_OK);
    btnImportar.Enabled := False;
